...
static uint8_t CanRead_E0(void);
static uint8_t ReadByte_E0(void);
static uint8_t CanWrite_E0(void);
static void    WriteByte_E0(uint8_t data);

/*! \brief  Send a byte to UARTE0
 *
 *  \param  data      byte to send
 *
 *  \return void
 */
inline void uartE0_putc(uint8_t data) 
{ 
  WriteByte_E0(data);
}

/*! \brief  Read a byte from UARTE0
 *
 *  \return Received byte from buffer or
 *          UART_NO_DATA if buffer is empty
 */
inline uint16_t uartE0_getc(void) 
{
  uint8_t data;
...
/*  \brief  Send a character to UARTE0 
 ...
 */
static int uartE0_fputc(char c, FILE *stream)
{
  if ( ! CanWrite_E0() ) return 1;
...
/*  \brief  Read character fromUARTE0 
 *          a static function necessary for standard stream
 *
 *  \param  stream   file stream
 *
 ...
 */
static int uartE0_fgetc(FILE * stream)
{
  int c;
...
FILE uartE0_stdinout = FDEV_SETUP_STREAM(uartE0_fputc, uartE0_fgetc, _FDEV_SETUP_RW);   //!< FILE structure for standard streams
...
void init_stream(uint32_t f_cpu)
{
  ...
  bsel   = calc_bsel(f_cpu, BAUD_115K2, bscale, UART_NO_DOUBLE_CLK);
	...
	PORTE.PIN2CTRL = PORT_OPC_PULLUP_gc;  // pullup on rx
...
static volatile uint8_t tx_E0_wridx, tx_E0_rdidx, tx_E0_buf[TXBUF_DEPTH_E0];
static volatile uint8_t rx_E0_wridx, rx_E0_rdidx, rx_E0_buf[RXBUF_DEPTH_E0];
 ...
 */
static uint8_t CanRead_E0(void) {
	uint8_t wridx = rx_E0_wridx, rdidx = rx_E0_rdidx;
	
	if(wridx >= rdidx)
		return wridx - rdidx;
	else
		return wridx - rdidx + RXBUF_DEPTH_E0;
...
static uint8_t ReadByte_E0(void) {
	...
	res = rx_E0_buf[curSlot];

	nextSlot = curSlot + 1;
	if(nextSlot >= RXBUF_DEPTH_E0)
		nextSlot = 0;
	rx_E0_rdidx = nextSlot;
 ...
 */
static uint8_t CanWrite_E0(void) {
	uint8_t wridx1 = tx_E0_wridx + 1, rdidx = tx_E0_rdidx;
	
	if(wridx1 >= TXBUF_DEPTH_E0)
		wridx1 -= TXBUF_DEPTH_E0;
	if(rdidx >= wridx1)
		return rdidx - wridx1;
	else
		return rdidx - wridx1 + TXBUF_DEPTH_E0;
...
static void WriteByte_E0(uint8_t data) {
	...
	while(!CanWrite_E0())
		USARTE0.CTRLA = USART_RXCINTLVL_MED_gc | USART_TXCINTLVL_OFF_gc | USART_DREINTLVL_LO_gc;
	
	curSlot = tx_E0_wridx;
	tx_E0_buf[curSlot] = data;
	
	nextSlot = curSlot + 1;
	if(nextSlot >= TXBUF_DEPTH_E0)
		nextSlot = 0;

	savePMIC = PMIC.CTRL;
	PMIC.CTRL = savePMIC & ~PMIC_LOLVLEN_bm;
	tx_E0_wridx = nextSlot;
	USARTE0.CTRLA = USART_RXCINTLVL_MED_gc | USART_TXCINTLVL_OFF_gc | USART_DREINTLVL_LO_gc;
	PMIC.CTRL = savePMIC;

} // WriteByte_E0 

/*  \brief  ISR for receiving bytes from UARTE0.
...
ISR(USARTE0_RXC_vect) {
	...
	curSlot = rx_E0_wridx;
	rx_E0_buf[curSlot] = USARTE0.DATA;
	
	nextSlot = curSlot + 1;
	if(nextSlot >= RXBUF_DEPTH_E0)
	nextSlot = 0;
	
	if(nextSlot != rx_E0_rdidx)
	rx_E0_wridx = nextSlot;
	
} // ISR(USARTE0_RXC_vect)


/*  \brief  ISR for transmitting bytes to UARTE0.
...
ISR(USARTE0_DRE_vect) {
	...
	nextSlot = curSlot = tx_E0_rdidx;
	lastSlot = tx_E0_wridx;
	
	if(curSlot != lastSlot) {
		USARTE0.DATA = tx_E0_buf[curSlot];
		nextSlot = curSlot + 1;
		if(nextSlot >= TXBUF_DEPTH_E0)
		nextSlot = 0;
	}
	if(nextSlot == lastSlot)
	USARTE0.CTRLA = USART_RXCINTLVL_MED_gc | USART_TXCINTLVL_OFF_gc | USART_DREINTLVL_OFF_gc;
	
	tx_E0_rdidx = nextSlot;
